...
func findToolTipLayer(handle *ToolTipHandle) *ToolTipLayer {
	...
	if handle.overlay != nil {
		if sub := tl.overlays[handle.overlay]; sub != nil {
			tl = sub
		}
		// No layer for this overlay (e.g. Fyne dialog OverlayContainer): use the root window layer
		// so tooltips still work instead of logging an error.
	}
	return tl
}
